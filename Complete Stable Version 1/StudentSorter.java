...
public class StudentSorter {
	...
	public ArrayList<ArrayList<Student>> sort() {
		...
		for(int i=0;i<5;i++) {
			...
			boolean valid = false;
			while(!valid) {
				groups = singleDaySort();//sort each day individually
				boolean good = true;
				for(Group g:groups) {
					if(!g.isValid()) {
						good = false;
				...
				if(good) {
					...
					for(int k=0; k<blacklist.size(); k++) {
						blacklist.get(k).setExclude(myStudents.get(k));
					}
					valid=true;
				} else {
					tries++;
	 ...
	 */	
	public ArrayList<Group> singleDaySort() {//for one day
		...
		try {
			...
			for(Student s:myStudents) {
				...
				int x = 0;
				while(x<groups.size()&&!groups.get(x).isOpen(s)) {//must refine this so that students get placed correctly!
